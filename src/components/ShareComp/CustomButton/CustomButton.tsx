...
const CustomButton: React.FC<ButtonType> = ({ title, link }) => {
    return (
        <Link href={link} className="group bg-white hover:bg-[#051a2f] text-[#051a2f] hover:text-white font-bold rounded-full px-3 md:px-7 py-2 border-4 border-transparent hover:border-white uppercase text-sm lg:text-lg flex items-center gap-2 transition-all duration-300" >
            <span>{title}</span>
